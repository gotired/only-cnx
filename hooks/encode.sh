#!/usr/bin/env bash
# Only CNX — Stop encode-enforcement hook.
# Makes durable memory auto-write reliable: if a run scratchpad (.only-cnx/run/) is
...
vault=""
if is_valid_vault "${ONLY_CNX_MEMORY:-}"; then
  vault="$ONLY_CNX_MEMORY"
elif is_valid_vault "$proj/.only-cnx/memory"; then
  vault="$proj/.only-cnx/memory"
fi
...
if stop_active:
    print(json.dumps({
        "continue": True,
        "systemMessage": "Only CNX: finishing — verify durable learnings were saved to the vault and "
                         ".only-cnx/run/ was rolled."
    }))
    sys.exit(0)

reason = (
    "Only CNX team memory has not been encoded yet — a run scratchpad (.only-cnx/run/) is still present "
    ...
    "templates/); (2) update MEMORY.md with one index line per note; (3) ONLY THEN delete .only-cnx/run/ as "
