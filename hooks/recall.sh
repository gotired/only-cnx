#!/usr/bin/env bash
# Only CNX — SessionStart recall hook.
# Guarantees the team recalls durable memory: if a valid team vault exists in this
# project (or via $ONLY_CNX_MEMORY), injects its MEMORY.md index into the model's context.
...
if is_valid_vault "${ONLY_CNX_MEMORY:-}"; then
  ...
  if is_valid_vault "$proj/.only-cnx/memory"; then
...
ctx = (
    "Only CNX team memory is available at `%s`. Today is %s.\n"
    ...
    "templates/daily.md if missing), update MEMORY.md, then roll `.only-cnx/run/`. Never write secret values.\n\n"
